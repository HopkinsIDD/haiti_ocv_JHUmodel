## pomp model in C for the 2-department vaccination campaign
...
rSim.step <- Csnippet('
  ...
  double foi = pow(I+I1+I2+(1-kappa)*(A+A1+A2), nu)*mybeta/N;  
  ...
  reulermultinom(4,S,&Srate[0],dt,&Strans[0]);
  reulermultinom(5,E,&Erate[0],dt,&Etrans[0]);
  reulermultinom(4,I,&Irate[0],dt,&Itrans[0]);
  reulermultinom(4,A,&Arate[0],dt,&Atrans[0]);
  reulermultinom(4,R,&Rrate[0],dt,&Rtrans[0]);
  ...
  S += -Strans[0] - Strans[1] - Strans[2] - Strans[3] + Rtrans[0] + births;
  E += -Etrans[0] - Etrans[1] - Etrans[2] - Etrans[3] - Etrans[4] + Strans[0];
  I += -Itrans[0] - Itrans[1] - Itrans[2] - Itrans[3] + Etrans[0];
  A += -Atrans[0] - Atrans[1] - Atrans[2] - Atrans[3] + Etrans[1];
  R += -Rtrans[0] - Rtrans[1] - Rtrans[2] - Rtrans[3] + Itrans[0] + Atrans[0];
  ...
  incid += Etrans[0] + E1trans[0] + E2trans[0]; // incidence is cumulative entries into I state
  incidU += Etrans[0];
  incidV += E1trans[0] + E2trans[0];
  asymV += E1trans[1] + E2trans[1];
  newV += Strans[2] + Strans[3] + Etrans[3] + Etrans[4] + Itrans[2] + Itrans[3] + Atrans[2] + Atrans[3] + Rtrans[2] + Rtrans[3];
  foival += foi;
  Str0 += Strans[0];
...
build.fc.mod <- function(pop, ##  = get.haiti.pop()
  ...
  my.mod <- pomp(
    ...
    covar = covar,
    tcovar = covar.times, 
    statenames=c("S", sprintf("S%d", 1:2),
                 "E", sprintf("E%d", 1:2),
                 "I", sprintf("I%d", 1:2),
                 "A", sprintf("A%d", 1:2),
                 "R", sprintf("R%d", 1:2),
    ...
    initializer=function(params, t0, ...){
      all.state.names <- c("S", sprintf("S%d", 1:2),
                          "E", sprintf("E%d", 1:2),
                          "I", sprintf("I%d", 1:2),
                          "A", sprintf("A%d", 1:2),
                          "R", sprintf("R%d", 1:2),
      ...
      x0[c("S","S1","S2",
           ...
           "I","I1","I2",
           "A","A1","A2",
           "R","R1","R2")] <- round(initpop*fracs/sum(fracs)) ## rm incid from statenames
