...
if(plot_modules){
  ...
  scale_x_date(limits = c(max(plotdat_f$weekdate)-300, max(plotdat_f$weekdate))) +
  scale_y_continuous(limits = c(0, 2000))
  ggsave(paste0(fig_dir, "o_ffullzm_if_", episettings$str_if, "_", endsettings$str_if, ".pdf"), plt_ofzm, width = 6, height = 4, units = "in")
  print(plt_ofzm)
...
if(!file.exists(paste0(out_dir_fc, "fcelim_if_", fcsettings$str_if, ".rds"))){
  fcelim <- process_fc_elim_novac(fn_fc_elim_full)
